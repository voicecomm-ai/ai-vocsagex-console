...
import cn.voicecomm.ai.voicesagex.console.api.nebula.enums.AttributeEnum;
import cn.voicecomm.ai.voicesagex.console.api.nebula.enums.EdgeDirectionEnum;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.AttributeBean;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphAddAttribute;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphCreateIndex;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphCreateTagEdge;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphDelAttribute;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphDeleteEdge;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphDeleteVertex;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphDropAttribute;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphExpand;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphInsertEdge;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphPageAttribute;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphPageEdge;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphShowAttribute;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphShowIndex;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphShowInfo;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphSpace;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphUpdateEdge;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphUpdateVertex;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphVertexPathQuery;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphVertexTatAttributeQuery;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.GraphVertexTatsQuery;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.DO.PropertyBean;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.GraphCreateSpace;
import cn.voicecomm.ai.voicesagex.console.api.nebula.db.po.GraphCreateVertex;
...
public class NebulaUtil {
  ...
  public static final StringBuffer STRING_BUFFER_TAG_LIST = new StringBuffer();
  ...
  public static String showAttributes(GraphShowAttribute graphShowAttribute) {
    ...
    if (AttributeEnum.SPACES.name().equalsIgnoreCase(graphShowAttribute.getAttribute())) {
      ...
      showSpaces = String.format("USE `%s`;SHOW %s;", graphShowAttribute.getSpace(),
  ...
  public static String showIndexes(GraphShowIndex graphShowIndex) {
    String showIndexes = String.format("USE `%s`;SHOW %s INDEXES;", graphShowIndex.getSpace(),
        graphShowIndex.getAttribute());
  ...
  public static String createSpace(GraphCreateSpace graphCreateSpace) {
    String createSpace = String.format(
        "CREATE SPACE `%s` (partition_num = %s, replica_factor = %s,vid_type = %s %s) COMMENT = '%s'",
        graphCreateSpace.getSpace(), graphCreateSpace.getPartitionNum(),
        graphCreateSpace.getReplicaFactor(), graphCreateSpace.getFixedType(),
  ...
  public static String createTagEdge(GraphCreateTagEdge graphCreateTagEdge) {
    ...
    if (CollectionUtil.isNotEmpty(propertyList)) {
      for (int i = 0; i < propertyList.size(); i++) {
        ...
        stringBuffer.append(
            " `" + propertyBean.getPropertyName() + "` " + propertyBean.getPropertyType() + " " + ""
                + propertyBean.getIsNull() + " " + propertyBean.getDefaultValue() + " COMMENT '"
                + propertyBean.getPropertyComment() + "' ");
    ...
    String createTag =
        "USE `" + graphCreateTagEdge.getSpace() + "`;CREATE " + graphCreateTagEdge.getType() + " `"
            + graphCreateTagEdge.getTagEdgeName() + "` " + "( " + bufferString + " )  COMMENT = '"
            + graphCreateTagEdge.getTagEdgeComment() + "' ";
  ...
  public static String createPoint(GraphCreateVertex graphCreateVertex, String vidType) {
    ...
    String createPoint = String.format(
        "USE `%s`;INSERT VERTEX IF NOT EXISTS `%s`(%s) VALUES %s: (" + bufferString + ");",
        graphCreateVertex.getSpace(), graphCreateVertex.getTagName(), STRING_BUFFER_TAG_LIST,
        pointKey);
  ...
  public static String dropAttribute(GraphDropAttribute graphDropAttribute) {
    String dropAttribute = String.format("USE `%s`;DROP %s IF EXISTS `%s`;",
        graphDropAttribute.getSpace(), graphDropAttribute.getAttribute(),
        graphDropAttribute.getAttributeName());
  ...
  public static String dropIndex(GraphDropAttribute graphDropAttribute) {
    String dropIndex = String.format("USE `%s`;DROP %s INDEX IF EXISTS `%s`;",
        graphDropAttribute.getSpace(), graphDropAttribute.getAttribute(),
        graphDropAttribute.getAttributeName());
  ...
  public static String addAttributeProperty(GraphAddAttribute graphAddAttribute) {
    String addAttributeProperty = String.format("USE `%s`;ALTER %s `%s` ADD (`%s` %s %s %s %s);",
        graphAddAttribute.getSpace(), graphAddAttribute.getAttribute(),
        graphAddAttribute.getAttributeName(), graphAddAttribute.getPropertyName(),
        graphAddAttribute.getPropertyType(), graphAddAttribute.getIsNull(),
        graphAddAttribute.getDefaultValue(), graphAddAttribute.getCommon());
  ...
  public static String queryMatch(String space) {
    String queryMatch = String.format(
        "USE `%s`; match (v) return v limit " + Integer.MAX_VALUE + ";", space);
    log.info("match查询 -gql语句:{}", queryMatch);
    return queryMatch;
  }
  ...
  public static String deleteVertex(GraphDeleteVertex graphDeleteVertex, String vidType) {
    ...
    String deleteVertex = String.format("USE `%s`; DELETE VERTEX %s;", graphDeleteVertex.getSpace(),
        vid);
  ...
  public static String deleteVertexEdge(GraphDeleteVertex graphDeleteVertex, String vidType) {
    ...
    String deleteVertexEdge = String.format("USE `%s`; DELETE VERTEX %s WITH EDGE;",
        graphDeleteVertex.getSpace(), vid);
  ...
  public static String insertEdge(GraphInsertEdge graphInsertEdge, String vidType) {
    ...
    String insertEdge = String.format(
        "USE `%s`; INSERT EDGE IF NOT EXISTS `%s` (%s) VALUES %s->%s:( %s );",
        graphInsertEdge.getSpace(), graphInsertEdge.getEdgeName(),
        StrUtil.isNotBlank(str) ? str.substring(0, str.length() - 1) : "", srcVid, dstVid,
        bufferString);
    ...
    if (CollectionUtil.isNotEmpty(edgeValueList)) {
      ...
      for (int i = 0; i < edgeValueList.size(); i++) {
        ...
        if (value instanceof String) {
          stringBuffer.append("'").append(StringEscapeUtils.unescapeHtml((String) value))
              .append("'");
        } else {
          stringBuffer.append(value);
        }
  ...
  public static String listEdge(GraphSpace graphSpace) {
    String listEdge = String.format(
        "USE `%s`;MATCH ()-[e]->()RETURN e LIMIT " + Integer.MAX_VALUE + " ;",
        graphSpace.getSpace());
  ...
  public static String expandQuery(GraphExpand graphExpand, String vidType) {
    ...
    String expandQuery = String.format("USE `%s`;MATCH p=(v) %s- [e " + bufferString
            + " * %s %s]-%s (v2) WHERE id(v) IN [%s]  %s RETURN p LIMIT " + graphExpand.getResultSize()
            + ";", graphExpand.getSpace(), leftAndRight.get(0), graphExpand.getStepStart(),
        graphExpand.getStepEndResult(), leftAndRight.get(1), graphExpand.getVidList(vidType),
  ...
  public static String subgraphQuery(GraphExpand graphExpand, String vidType) {
    ...
    String expandQuery = String.format(
        "USE `%s`;GET SUBGRAPH %s STEPS FROM %s %s %s YIELD VERTICES AS `vertices_`, EDGES AS `edges_`;",
        graphExpand.getSpace(), graphExpand.getStepStart(), graphExpand.getVidList(vidType),
        direction, Joiner.on(",").join(graphExpand.getEdgeList()));
    log.info("扩展查询 -gql语句:{}", expandQuery);
    return expandQuery;
  }
  ...
  public static String vertexTagsQuery(GraphVertexTatsQuery graphVertexTatsQuery, String vidType) {
    ...
    String vertexTagsQuery = String.format(
        "USE `%s`;MATCH p=(v %s) %s return v limit " + Integer.MAX_VALUE + ";",
        graphVertexTatsQuery.getSpace(), tag, pointKey);
    log.info("根据tag标签查询点 -gql语句:{}", vertexTagsQuery);
    return vertexTagsQuery;
  }

  public static String vertexTagsQueryPage(GraphVertexTatsQuery graphVertexTatsQuery,
      String vidType) {
    ...
    String vertexTagsQuery = String.format(
        "USE `%s`;MATCH p=(v %s) %s return v SKIP " + skip + " limit "
            + graphVertexTatsQuery.getPageSize() + ";", graphVertexTatsQuery.getSpace(),
//Joiner.on(":").join(graphVertexTatsQuery.getTagList()));
   ...
   **/
  public static String vertexTagAttributeQuery(
      GraphVertexTatAttributeQuery graphVertexTatAttributeQuery) {
    String vertexTagsQuery = String.format("USE `%s`;match p=(v:`%s`) %s return v limit "
            + graphVertexTatAttributeQuery.getResultSize() + ";",
        graphVertexTatAttributeQuery.getSpace(), graphVertexTatAttributeQuery.getTag(),
        graphVertexTatAttributeQuery.getCondition());
  ...
  public static String createIndex(GraphCreateIndex graphCreateIndex) {
    ...
    String vertexTagsQuery = String.format(
        "USE `%s`;CREATE %s INDEX `%s` on `%s`(%s) COMMENT '%s';", graphCreateIndex.getSpace(),
        graphCreateIndex.getType(), graphCreateIndex.getIndexName(),
        graphCreateIndex.getTagEdgeName(), Joiner.on(",").join(propertyList),
        graphCreateIndex.getComment());
  ...
  public static String createFullIndex(GraphCreateIndex graphCreateIndex) {
    ...
    String vertexTagsQuery = String.format(
        "USE `%s`;CREATE FULLTEXT %s INDEX nebula_index_" + RandomUtil.randomNumbers(4)
            + " ON `%s`(name)", graphCreateIndex.getSpace(), graphCreateIndex.getType(),
        graphCreateIndex.getTagEdgeName());
  ...
  public static String showAttributePage(GraphPageAttribute graphPageAttribute) {
    ...
    if (AttributeEnum.EDGES.name().equalsIgnoreCase(graphPageAttribute.getAttribute())) {
      String showAttributePage = String.format(
          "USE `%s`;MATCH ()-[e]->() RETURN e SKIP %s LIMIT %s;", graphPageAttribute.getSpace(),
          skip, graphPageAttribute.getPageSize());
...
//    }
  ...
  public static String edgePage(GraphPageEdge graphPageEdge) {
    ...
    if (StrUtil.isBlank(graphPageEdge.getVid())) {
      edgePage = String.format("USE `%s`;MATCH ()-[e %s]->()RETURN e SKIP " + skip + " LIMIT "
          + graphPageEdge.getPageSize() + " ;", graphPageEdge.getSpace(), edge);
    } else {
      edgePage = String.format("USE `%s`;MATCH (m)-[e %s]->(n)  where id(m)=='%s' RETURN e "
              + "UNION MATCH (m)-[e %s]->(n)  where id(n)=='%s' RETURN e SKIP " + skip + " LIMIT "
              + graphPageEdge.getPageSize() + " ;", graphPageEdge.getSpace(), edge,
          graphPageEdge.getVid(), edge, graphPageEdge.getVid());
    }
  ...
  public static String vertexPage(GraphSpace graphSpace) {
    ...
    String queryMatch = String.format(
        "USE `%s`; match (v) return v skip " + skip + " limit " + graphSpace.getPageSize() + ";",
        graphSpace.getSpace());
  ...
  public static String updateEdge(GraphUpdateEdge graphUpdateEdge, String vidType) {
    ...
    String updateVertex = String.format("USE `%s`; UPDATE EDGE ON `%s` %s -> %s@0 SET %s;",
        graphUpdateEdge.getSpace(), graphUpdateEdge.getEdgeName(), srcVid, dstVid, stringBuffer);
  ...
  public static String vertexAskQueryAnalysis(String tagName, String name) {
    String vertexTagsQuery = String.format(
        "match p=(v:`%s`)-[e]-(v2) where v.`%s`.`name` CONTAINS '%s' return nodes(p),relationships(p) ",
  ...
  public static String vertexAskQuery(String tagName, String name) {
    String vertexTagsQuery = String.format(
        "match p=(v:`%s`) where v.`%s`.`name` CONTAINS '%s' return nodes(p),relationships(p) ",
        tagName, tagName, name);
    log.info("根据tag标签属性查询点 -gql语句:{}", vertexTagsQuery);
    return vertexTagsQuery;
  }
  ...
  public static String edgeCondition(GraphPageEdge graphPageEdge) {
    ...
    if (StrUtil.isBlank(graphPageEdge.getVid())) {
      edgeCondition = String.format(
          "USE `%s`;MATCH ()-[e %s]->() RETURN e limit " + Integer.MAX_VALUE + ";",
          graphPageEdge.getSpace(), edge);
    } else {
      edgeCondition = String.format("USE `%s`;MATCH (m)-[e %s]->(n)  where id(m)=='%s' RETURN e "
              + "UNION MATCH (m)-[e %s]->(n)  where id(n)=='%s' RETURN e LIMIT " + Integer.MAX_VALUE
              + ";", graphPageEdge.getSpace(), edge, graphPageEdge.getVid(), edge,
          graphPageEdge.getVid());
    }
  ...
  public static String fullQuery(String tag, String word) {
    String fullQuery = String.format(
        "LOOKUP ON `%s` WHERE WILDCARD(`%s`.name, \"*%s*\") YIELD id(vertex)", tag, tag, word);
    return fullQuery;
  }
  ...
  public static String queryMatchVertex(String space, List<?> idList) {
    String queryMatch = String.format("USE `%s`; match (v)  WHERE id(v) IN %s RETURN v;", space,
        JSONUtil.toJsonStr(idList));
    log.info("match查询 -gql语句:{}", queryMatch);
    return queryMatch;
  }
  ...
  public static String findPath(GraphVertexPathQuery graphVertexPathQuery, String vidType) {
    ...
    String queryMatch = String.format(
        "USE `%s`; FIND %s PATH WITH PROP FROM %s TO %s OVER %s %s %s UPTO %s STEPS YIELD path AS p | limit %s;",
        graphVertexPathQuery.getSpace(), graphVertexPathQuery.getPathType(),
        srcVid.substring(0, srcVid.length() - 1), dstVid.substring(0, dstVid.length() - 1),
        graphVertexPathQuery.getEdgeList(), graphVertexPathQuery.getDirect(),
        StringEscapeUtils.unescapeHtml(graphVertexPathQuery.getCondition()),
  ...
  public static String vertexAskQueryByTag(String tag, List<String> ids) {
    String vertexAskQueryByTag = String.format(
        " MATCH p=(v:`%s`)-[e]->(v2) WHERE id(v2) IN %s  RETURN nodes(p) LIMIT 1000", tag,
        JSONUtil.toJsonStr(ids));
  ...
  public static String commonNeighbor(String space, List<Object> ids) {
    ...
    for (int i = 0; i < ids.size(); i++) {
      if (i + 1 < ids.size()) {
        stringBuffer.append(" UNION GO FROM '" + ids.get(i)
            + "' OVER * REVERSELY YIELD src(edge) as id) INTERSECT(GO FROM '" + ids.get(i + 1)
            + "' OVER *  YIELD dst(edge) as id ");
      }
    }
    log.info(stringBuffer.toString());
    String commonNeighbor = String.format(
        "USE `%s`; (GO FROM '%s' OVER *  YIELD dst(edge) as id  %s UNION GO FROM '%s' OVER * REVERSELY YIELD src(edge) as id)",
        space, ids.get(0), stringBuffer, ids.get(ids.size() - 1));
    log.info("共同邻居 -gql语句:{}", commonNeighbor);
    return commonNeighbor;
  }

  public static String findPathList(String space, List<String> row, List<Object> ids,
      String vidType) {
    String str = JSONUtil.toJsonStr(row);
    String idsStr = JSONUtil.toJsonStr(ids);
    String findPathList = String.format(
        "USE `%s`; FIND ALL PATH WITH PROP FROM %s TO %s OVER * BIDIRECT UPTO 1 STEPS YIELD path as p | limit 1000;",
